...
5. Mixture of Experts (MoE)
    ...
    - SoftMoE
        ...
        $$
        [
        \text{SoftMoE}(x)=W\Big(\sum_k g_k(x) E_k(x)\Big)
        ]
        $$
...
Decoding = sampling the next token from model’s distribution:
$$
[
p(x_t \mid x_{<t})
]
...
### Beam Search [[code]](1.6.3.top_k_sampling.py)

Keeps top-(B) candidates:
$$
[
\text{Beam}*t = \text{TopB}\left( \prod*{\tau=1}^t p(x_\tau) \right)
]
$$
...
| PrefixLM        | Hybrid mask     | Prefix modeling | Reasoning, dialogue        | GLM               |
| MoE             | Routed Experts  | Sparse Compute  | Efficient scaling          | DeepSeek, Mistral |
