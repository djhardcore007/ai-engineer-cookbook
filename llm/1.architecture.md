...
- linear attention
    ...
    - math
        Below is a **clean, simple Markdown explanation** of **linear attention** and **why removing softmax helps**.

---

# **1. What Is Linear Attention?**

**Linear attention** is an alternative to standard (softmax) attention that reduces the computation cost from:
...
* ( n ) = sequence length
* Traditional Transformers compute an ( n \times n ) attention matrix
* Linear attention avoids this quadratic cost by **reordering the math** so that attention can be computed with efficient matrix multiplication.

---

# **2. The Standard (Softmax) Attention**
...
[
QK^\top \quad (\text{size } n \times n)
]

This is why regular Transformers are slow for long sequences.

---

# **3. The Idea of Linear Attention**

Linear attention **removes the softmax** and replaces it with a kernel function (\phi(\cdot)) that is **feature-map friendly**, such as:

[
\phi(x) = \text{ReLU}(x) + 1 \quad \text{(example)}
]

Then attention becomes:

[
\text{Att}(Q,K,V) = \phi(Q)\left(\phi(K)^\top V\right)
]

The trick:

* Compute ( S = \phi(K)^\top V ) once → cost ( O(n) )
* Multiply ( \phi(Q)S ) → cost ( O(n) )

No ( n \times n ) matrix is ever created.

---

# **4. Why Removing Softmax Helps**
