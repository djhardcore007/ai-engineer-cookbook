...
- Length Extrapolation
    ...
    Here is **one clean Markdown table** comparing **RoPE / NTK-aware RoPE / YaRN / ALiBi / RPE** — each with **math + intuition** in the same table.

        | Method                                | Math (Core Formula)                                                                                                                          | What It Encodes                      | Intuition (Why it works)                                                                                                             |
        | ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------ |
        | **RPE (Relative Position Embedding)** | $$\text{score}*{ij}=Q_iK_j^\top + a*{,i-j}$$                                                                                                 | Relative distance (i-j)              | Only cares about *how far apart* tokens are → naturally supports longer sequences.                                                   |
        | **RoPE (Rotary Positional Encoding)** | $$Q_t^{rot}=R_tQ_t,\quad K_t^{rot}=R_tK_t$$ $$R_t=\begin{bmatrix}\cos(\theta t)&-\sin(\theta t)\\sin(\theta t)&\cos(\theta t)\end{bmatrix}$$ | Encodes pairwise relative rotation   | Positions rotate Q/K in 2D space. Dot product changes with *angle difference* → relative position emerges implicitly.                |
        | **NTK-aware RoPE Scaling**            | $$\theta' = \theta / s$$ (where (s) = scaling factor)                                                                                        | Slow down RoPE rotation for long seq | RoPE rotates **too fast** at long range. NTK scaling slows angles so model stays stable beyond training length.                      |
        | **YaRN (Yet Another RoPE Extension)** | $$\theta'=\theta \cdot \frac{1}{s_{\text{base}}} + g(t)$$ (mixture of base-scaling + grow function)                                          | Stable long-range extension of RoPE  | Combines **slow rotation** + **boost at large positions**. Prevents angle saturation AND under-rotation → best long-seq RoPE so far. |
        | **ALiBi (Linear Bias)**               | $$\text{score}_{ij} += w_h (i-j)$$                                                                                                           | Linear distance penalty              | Adds a **soft mask**: far tokens get linearly lower scores, close tokens favored. Generalizes *perfectly* to long sequences.         |
